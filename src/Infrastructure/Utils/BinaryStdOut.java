/* Created by andreea on 14/04/2020 */
...
public final class BinaryStdOut {
     ...
     * Writes the specified bit to standard output.
     */
    private static void writeBit(String fileName, boolean bit) {
        if (!isInitialized) initialize(fileName);
     ...
     */
    private static void writeByte(String fileName, int x) {
        if (!isInitialized) initialize(fileName);
     ...
     * @param x the {@code boolean} to write.
     */
    public static void write(String fileName, boolean x) {
        writeBit(fileName, x);
    }
     ...
     */
    public static void write(String fileName, byte x) {
        writeByte(fileName, x & 0xff);
    }
    ...
    public static void write(String fileName, int x) {
        writeByte(fileName, (x >>> 24) & 0xff);
        writeByte(fileName, (x >>> 16) & 0xff);
        writeByte(fileName, (x >>>  8) & 0xff);
        writeByte(fileName, (x >>>  0) & 0xff);
    }
     ...
     */
    public static void write(int r, String s) {
        for (int i = 0; i < s.length(); i++)
            write(s.charAt(i), r);
    }
}
